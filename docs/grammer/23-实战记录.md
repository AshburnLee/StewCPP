...
要避免在 std::map 中插入空值的 key，需要*先判断*，用 std::map 的 find 函数来检查 key 是否存在，然后再进行访问操作.
...
- **内置 `!` 对算术类型**：编译器会先把操作数**隐式转换为 `bool`**（0 => false，非 0 → true），再取反；`!a` 的类型已是 `bool`，故 `!!a` 的第二次 `!` 不再做「整型→`bool`」，只对 `bool` 取反。
...
`std::ref(value)` 将 `value` 的引用包装为一个可传递给函数的对象，这样可以确保 func 函数中对 `value` 的修改会影响到实际的对象，避免了*不必要的拷贝*开销。
...
0xffffffffffffffff 表示一个十六进制数（16进制中最大的数），它等价于十进制的 18,446,744,073,709,551,615。 它的类型取决于上下文，但最常见的是 unsigned long long 类型 64位整数。
...
- `reinterpret_cast`: 执行低级别的**位模式**重新解释。它不进行任何类型检查，直接将一个指针类型的位模式**解释为**另一个指针类型。这非常危险，因为它可能导致程序崩溃或产生不可预测的结果，尤其是在涉及不同数据类型大小或对齐方式时。
...
fn/name/threshold 通常都变成对 tuple 成员的*引用*，*不会拷贝* std::string。

`auto&&` 如何理解？它创建了一个所谓的*通用引用*（forwarding reference）。通用引用是一个*类型推导*上下文中的右值引用，它可以根据初始化表达式是*左值还是右值*来*绑定*到左值或右值。
...
~~~cpp
int x = 42; // x是左值，42是右值
auto&& ref1 = x; // ref1 是左值引用，类型是int&，即 ref1 引用的左值x
auto&& ref2 = 42; // ref2 是右值引用，类型是int&&，即 ref2 引用了右值42
~~~

`auto&&` 会因为“左值范围”而*折叠*成左值引用（T&& & → T&）

`auto&&` 的价值是“写法更泛化”，避免拷贝，结构化绑定（C++17）：想这个例子中的 [fn, name, threshold] 把 tuple 拆开。

总结，关键是 “结构化绑定 + auto&& 让拆出来的成员尽量以引用方式工作，从而避免不必要拷贝，并在可能的右值场景下保留移动机会。”
