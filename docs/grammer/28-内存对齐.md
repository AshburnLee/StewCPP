...
CPU 根据编译器生成的机器码，将这些高级语言的**内存访问操作**转换为**对地址总线**的操作。
...
所以，将一个数据尽量放在步长之内，这避免了跨步长存储（这是结果），这成为**内存对齐**。
...
不考虑内存对齐，对象 t 占 4 + 4 + 1 = 9 个字节，但是为了内存对齐，c 实际占4字节，额外的 3 个会进行内存填充padding，将其浪费掉。编译器用空间换时间。
...
很多指令或数据访问在*对齐*时更安全/更快。

起始地址按 16 字节对齐（地址是 16 的倍数）

内存地址（十六进制）:
`... 0x1000 0x1001 0x1002 ... 0x100F 0x1010 0x1011 ...`

判断方法：`地址 % 16 == 0`。

16 字节对齐的“起始地址”只能放在： `0x1000, 0x1010, 0x1020, 0x1030, ...`, `alignas(16)` 就是要求变量起始地址像这些点
...
~~~cpp
alignas(16) int x; // x 的起始地址将是 16 的倍数  这句话什么意思？
/*
...
void* my_malloc(size_t size) {
    ...
    size = do_align(size);  // 手动对齐请求的内存大小
...
注意：数组名会**隐式地转换**为指向其第一个元素的指针。*** `char pool[SIZE];` 是一个数组，`pool[0]` 等价于 `*(pool + 0)`。
...
for (size_t i = 0; i <= 20; ++i) {
    std::cout << "do_align(" << i << ") = " << do_align(i) << std::endl;
}
~~~

这个函数作用是**将给定的 size 调整为一个对齐到指定字节边界的大小**。某些硬件要求数据必须对齐到 4 字节或 8 字节。**没有对齐的数据需要额外的内存访问操作**。
...
## 上述 do_align() 保证了对齐，那么如何判断内存中某个对象是否对齐?

判断思路：取地址 => 转成整数 => 看是否是 N 的整数倍。
...
struct S { double x; };
S s;
bool ok = is_aligned<alignof(S)>(&s);
~~~
