...
手动管理线程，每个线程跑一个函数；工程里通常会再封装成线程池。技术点：
- std::thread / std::jthread
- std::mutex / std::lock_guard / std::unique_lock
- std::condition_variable
- 线程池：任务队列 + worker 线程循环 pop + 执行

实例：主线程不断接受请求，把任务丢进任务队列。模型直观。

## 2. 任务/未来模型（适合少量“大任务”）

把“要做的事”当成一个任务，不显式管线程，拿一个 future 表示结果。技术点：
- std::async 启动任务
- std::future / std::promise
- 有时配合线程池或调度器替代裸 std::async

## 3. 数据并行

对容器/范围做同一操作，交给库内部决定是否/如何并行。
