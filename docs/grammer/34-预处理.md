...
预处理器指令不属于 C++ 语言本身，而是**编译过程**的一部分。包括条件编译、宏定义等、文件包含等。

是在 .cpp/.hpp 里用 `#` 开头写的那写内容。C++ 标准里只有“与处理指令”

KAQ：`target_compile_definitions(my_target PRIVATE MY_FEATURE=1)` 定义的变量属于预处理指令码？

非也，target_compile_definitions => 生成（宏）-DMY_FEATURE=1 => 预处理**器**看到 `#if MY_FEATURE` 时生效，**宏本身不是预处理指令，但它驱动预处理配置**。

当我写了 `target_compile_definitions(my_target PRIVATE MY_FEATURE=1)`，编译器接受到这个选项，相当于在**每一个** .cpp 文件头偷偷添加了：
...
用户不能在 C++ 里“再定义一种新的预处理指令”，只能用现有这几种。CMake 等构建系统只是帮开发者生成所有预处理指令要用到的宏。宏本身不是预处理指令。

所以**构建系统定义的 MY_FEATURE 是宏**。他**不是预处理指令**。
...
## 所以通过宏定义一个 print 和 定义一个 print 函数，这两种方式的优劣？
