...
实战代码见 `headers/grammer/overload.h`；与 **llama repo** 中的相关工程实践见 `source/operators_from_llama_demo.cpp`
...
### KAQ： `m_ptr[i]` 应该是 `int` 吧，为什么`operator[]`返回值类型是 `int&`  ***

表达式 `m_ptr[i]` 在类型上是一个 “指向（绑定到）第 i 个 `int` 的引用”，不是“把那个 `int` 拷贝一份出来”。

为什么标准要设计成 `int&` 而不是 `int`？

1. 若 `operator[]` 返回 `int`（按值）：你得到的是副本，和容器里那一位置脱钩；不能写 `m_ptr[i] = 99`（不能给临时副本赋回去）。

2. 返回 `int&` 的目的就是：让 `m_ptr[i]` 仍然代表容器里那一格本身，这样：读：需要值时，编译器会做 *左值到右值的转换*，从那一格读出一个 `int`；写：`m_ptr[i] = 99` 直接改那一格。

所以 容器中元素类型是 `int`，表达式 `m_ptr[i]` 类型是 `int&`.
...
1. `operator=()` 的返回值类型是**左值引用** `Array&`，这样不但能够避免在返回数据时调用拷贝构造函数，还能够达到连续赋值的目的。下面的语句就是连续赋值： 
   ~~~cpp
   arr4 = arr3 = arr2 = arr1;
   ~~~
