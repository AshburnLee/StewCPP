...
- 悬空指针(*Dangling Pointer*)：指向*已经被释放*的内存空间的指针。为了避免这种情况，应该在不再需要时将指针设置为 nullptr 或者让其超出作用域自动销毁。

- 野指针(*Wild Pointer*)：是指*未初始化*的指针，即指针变量声明后未被赋予任何有效的内存地址。这种指针可能指向任意内存地址，甚至可能是随机的垃圾数据。
